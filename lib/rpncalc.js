...
RpnCalc.prototype.setDigitGrouping = function(enabled) {
  this.digitGrouping = enabled;
};

RpnCalc.prototype.loadState = function(state) {
  var self = this;
  this.digitGrouping = state.digitGrouping || false;
...
RpnCalc.prototype.digitgrouping = function() {
  if (this.stack.length == 0) {
    throw new Error('digitgrouping Error: Too few arguments');
  }
  var values = this.popValues(1);
  this.setDigitGrouping(parseInt(values[0]) == 0 ? false : true);
};
RpnCalc.prototype.digitgrouping.description = "Enables/Disables digit grouping based on S<sub>1</sub>";
...
RpnCalc.prototype.swap.description = "Swaps S<sub>1</sub> and S<sub>2</sub>";

RpnCalc.prototype.dup = function() {
  if (this.stack.length < 1) {
    throw new Error("Dup Error: Too few arguments");
  }
  this.push(this.stack[0]);
};
RpnCalc.prototype.dup.description = "Duplicates S<sub>1</sub>";
...
RpnCalc.prototype.mod = function() {
  if (this.stack.length < 2) {
    throw new Error("Mod Error: Too few arguments");
  }
  var args = this.popValuesConvertUnits(2);
  var result = args[0] % args[1];
  if (args.units) {
...
RpnCalc.prototype.mod.category = "Math";
RpnCalc.prototype.mod.altName = '%';
RpnCalc.prototype.mod.description = "Calculates S<sub>2</sub> % S<sub>1</sub>";
...
RpnCalc.prototype.tosi = function() {
  if (this.stack.length < 1) {
    throw new Error("convert Error: Too few arguments");
  }
  var val;
  var originalUnits = null;
  if (this.stack[0].units) {
    originalUnits = units.getUnits(this.stack[0].units);
    val = this.stack[0].getValueInUnits(originalUnits.aliasSuffix);
  } else {
    val = this.stack[0].getValue();
  }
  var siUnits = units.getSIUnits(val);
  if (siUnits) {
    var v = val / siUnits.standardMultiplier;
    this.pop(1);
    v = v + '_' + siUnits.unitPrefix;
    if (originalUnits) {
      v += originalUnits.aliasSuffix;
    }
    this.push(v);
  } else {
    throw new Error("convert Error: Could not find SI units");
  }
};
RpnCalc.prototype.tosi.category = "Unit Conversion";
RpnCalc.prototype.tosi.description = "Normalizes the SI prefix of S<sub>1</sub>";

RpnCalc.prototype.fromsi = function() {
  if (this.stack.length < 1) {
    throw new Error("convert Error: Too few arguments");
  }
  var val;
...
RpnCalc.prototype.fromsi.category = "Unit Conversion";
RpnCalc.prototype.fromsi.description = "Converts S<sub>1</sub> to a decimal";

RpnCalc.prototype.graph = function() {
  if (this.stack.length < 1) {
    throw new Error("graph Error: Too few arguments");
  }
  if (this.stack[0].type != 'expression') {
    throw new Error("graph Error: 1st arg must be expression");
  }
  var args = this.pop(1);
  var graphData = {
    fn: 'graph',
    equation: args[0].value
  };
  this.emit('graph', graphData);
  return graphData;
};
RpnCalc.prototype.graph.description = "Graphs equation at S<sub>1</sub>";
